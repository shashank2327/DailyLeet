...
class Solution {
    public int countPaths(int n, int[][] roads) {
        ...
        int mod = 1000000007;
        PriorityQueue<long[]> q = new PriorityQueue<>((a, b) -> Long.compare(a[0], b[0]));  // Use long
        q.add(new long[]{0, 0});
        long[] time = new long[n];  // Use long for distances
        ...
        while (!q.isEmpty()) {
            ...
            for (int[] x : adj.get(node)) {
                long nextTime = currTime + x[0];  // Use long

                if (nextTime < time[x[1]]) {
                    time[x[1]] = nextTime;
                    count[x[1]] = count[node];  // Reset count because a new shortest path is found
